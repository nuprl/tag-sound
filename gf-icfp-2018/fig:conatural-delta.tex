\lblextra{Language $\langC$}{ extends $\langM$:}{
  $\begin{array}{l@{~~}c@{~~}l}
    ...
    \\
    v & \BNFeq & i \mid \vpair{v}{v} \mid \vlam{x}{e} \mid \vlam{\tann{x}{\tau}}{e} \mid \vmonfun{(\tau\tarrow\tau)}{v} \mid \vmonpair{(\tpair{\tau}{\tau})}{v}
  \end{array}$
}
...
\\
  ...
  & \mbox{if $v_0 \sim i$ or $v_0 \sim \vpair{v}{v'}$ or $v_0 \sim \vmonpair{\tpair{\tau_0}{\tau_1}}{v'}$}
\\
  \eunop{v} & \rrCD & A
...
\\
  ...
  & \mbox{if $v \sim \vpair{v_0}{v_1}$ or $v \sim \vmonpair{\tpair{\tau_0'}{\tau_1'}}{v'}$}
\\
  \edyn{\tint}{i} & \rrCS & i
  &
\\
