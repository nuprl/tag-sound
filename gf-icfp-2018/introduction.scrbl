...
A second approach is to compile types to contracts that enforce a standard,
 compositional form of type soundness@~cite[st-sfp-2006 tf-dls-2006].
Programmers may use sound types to reason about the behavior of programs;
 however, the runtime cost of the enforcement may be prohibitively
...
@item{
  we model the optional, type-sound, and tag-sound approaches to migratory
   ...
   that enforce soundness at language boundaries;
}
@item{
