...
@;
 ...
 between statically and dynamically typed code, which is the natural
 generalization of traditional type soundness@~cite[st-sfp-2006 tf-dls-2006
 ...
 invalid results from callbacks. But, it may impose a huge run-time
...
@;
 ...
 interactions between statically typed and dynamically typed parts of the
 code. 
@; 
 Finally, a third approach is to @emph{compromise} between those two
 extremes and to check @emph{only} the @emph{type constructors} as late as
 absolutely needed according to the type annotations@~cite[vss-popl-2017].
...
@item{@emph{How do the logical implications of the three approaches compare}?
 ...
 Publications on the tag-checking approach@~cite[vss-popl-2017] often state
 a conventional type soundness theorem for the pair of languages or 
 articulate informal soundness claims.
...
@item{@emph{How do the three approaches compare with respect to performance}? 
 ...
 compared the performance of fully typed tag-checking implementations to
 that of untyped implementations and proposed generalizations based on
 these results@~cite[vss-popl-2017]; others studied the performance of a
 range of mixed-typed programs and implied order-of-magnitude differences
 @emph{across completely different programming
 languages}@~cite[gm-pepm-2018].
