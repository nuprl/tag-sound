...
(: ghost-blocks (-> (Option World) BSet))
(define (ghost-blocks w)
  (tetra-blocks (tetra-change-color (world-tetra (world-jump-down (assert w world?)))
                                    'gray)))

;; Move the world according to the given key event.
(: world-key-move (-> (Option World) (Option String) World))
