...
(define (shift-stretch-axes arr new-ds)
  (define old-ds (assert (array-shape arr) vector?))
  ...
  (define old-f (assert (unsafe-array-proc arr) procedure?))
  (unsafe-build-array
   new-ds
   (λ: ([new-js : Indexes])
     (let ([old-js  (old-js)])
       (let: loop : Float ([k : Integer  0])
         (cond [(k . < . old-dims)
                (define new-jk (assert (vector-ref new-js (+ k shift)) integer?))
                (define old-dk (assert (vector-ref old-ds k) integer?))
                (define old-jk (fxmodulo new-jk old-dk))
                (vector-set! old-js k old-jk)
                (loop (+ k 1))]
               [else  (assert (old-f old-js) flonum?)]))))) )

(: array-broadcast (-> (Option Array) (Option Indexes) Array))
(define (array-broadcast arr ds)
  (if (and (array? arr) (vector? ds)) (let ()
      (cond [(equal? ds (array-shape arr))  arr]
            [else  (define new-arr (shift-stretch-axes arr ds))
                   (if (or (array-strict? arr) ((assert (array-size new-arr) integer?) . fx<= . (assert (array-size arr) integer?)))
                       new-arr
...
(define (shape-insert-axes ds n)
  (vector-append ((inst make-vector (Option Integer)) n 1) ds))
...
(define (shape-permissive-broadcast ds1 ds2 dims fail)
  ...
  (let loop ([#{k : Integer} 0])
    (cond [(k . < . dims)
           (define dk1 (assert (vector-ref ds1 k) integer?))
           (define dk2 (assert (vector-ref ds2 k) integer?))
           (vector-set!
            new-ds k
...
(: array-shape-broadcast (case-> ((Option (Listof (Option Indexes))) -> Indexes)
                                 ((Option (Listof (Option Indexes))) (U #f #t 'permissive) -> Indexes)))
(define (array-shape-broadcast dss [broadcasting (array-broadcasting)])
  (if (list? dss) (let ()
      ...
      (cond [(empty? dss)  #()]
            [else  (for/fold ([new-ds  (assert (first dss) vector?)]) ([ds  (in-list (rest dss))])
                     (shape-broadcast2 new-ds (assert ds vector?) fail broadcasting))]))
    (error 'dynamic-typecheck)))
