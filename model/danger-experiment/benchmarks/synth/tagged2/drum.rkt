...
(define bass-drum
  (let ()
    ...
    (: vs (Vectorof (Option Flonum)))
    (define vs
      (for/vector : (Vectorof (Option Flonum))
                  #:length (assert (array-shape-size ds) integer?)
                  #:fill 0.0
...
(: drum (-> (Option Natural) (Option Pattern) (Option Natural) Array))
(define (drum n pattern tempo)
  (: samples-per-beat Natural)
  (define samples-per-beat (quotient (* fs 60) (assert tempo exact-nonnegative-integer?)))
  (: make-drum (-> Array Natural Array))
  (define (make-drum drum-sample samples-per-beat)
    (array-append*
     (list drum-sample
           (make-array (vector (- samples-per-beat
                                  (assert (array-size drum-sample) integer?)))
                       0.0))))
  (: O Array)
  ...
  (array-append*
   (for*/list : (Listof Array) ([i : Integer (in-range (assert n exact-nonnegative-integer?))]
                                        [beat : Drum-Symbol (in-list (assert pattern list?))])
     (case beat
       [(X)  X]
       [(O)  O]
       [(#f) pause]))))
