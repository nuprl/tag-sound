  ...
  (let*: ([ds : Indexes  ds-expr]
    ...
    (cond
      ...
      [else
       ...
       (case dims
         [(1)  (define: d0 : Integer (assert (vector-ref ds 0) integer?))
               (let: j0-loop : Void ([j0 : Integer  0])
                 (when (j0 . < . d0)
                   (vector-set! js 0 j0)
                   (f js j0)
                   (j0-loop (+ j0 1))))]
         [(2)  (define: d0 : Integer (assert (vector-ref ds 0) integer?))
               (define: d1 : Integer (assert (vector-ref ds 1) integer?))
               (let: j0-loop : Void ([j0 : Integer  0]
                                     [j : Integer  0])
                 (when (j0 . < . d0)
                   (vector-set! js 0 j0)
         ...
         [else  (let: i-loop : Integer ([i : Integer  0]
                                                   [j : Integer  0])
                  (cond [(i . < . dims)
                        (define: di : Integer (assert (vector-ref ds i) integer?))
  ...
  (let*: ([ds : Indexes  ds-expr]
    ...
    (define: size : Integer
      (let: loop : Integer ([k : Integer  0] [size : Integer  1])
        (cond [(k . < . dims)  (loop (+ k 1) (fx* size (assert (vector-ref ds k) integer?)))]
              [else  size])))
...
(: array-strict? ((Option Array) -> Boolean))
(define (array-strict? arr)
  (unbox (assert (Array-strict? (assert arr Array?)) box?)))

(: array-strict! (Array -> Void))
(define (array-strict! arr)
  (define strict? (assert (Array-strict? arr) box?))
  (unless (unbox strict?)
    ((assert (Array-strict! arr) procedure?))
    (set-box! strict? #t)))

(: array-default-strict! ((Option Array) -> Void))
...
(: unsafe-build-array ((Option (Vectorof (Option Integer))) (Option ((Vectorof (Option Integer)) -> (Option Float))) -> Array))
(define (unsafe-build-array ds f)
  (if (and (vector? ds) (procedure? f)) (let ()
      ...
      (let ([f  (box f)])
        ...
        (define (strict!)
          (let* ([old-f  : (-> (Vectorof (Option Integer)) (Option Float)) (unbox f)]
                 [vs     : (Vectorof (Option Float)) (inline-build-array-data ds (lambda (js j) (old-f js)) (Option Float))])
            ;; Make a new f that just indexes into vs
            (set-box! f (λ: ([js : (Option Indexes)])
                          (vector-ref vs (unsafe-array-index->value-index ds (assert js vector?)))))))
        (define unsafe-proc
          (λ: ([js : (Option Indexes)]) ((unbox f) (assert js vector?))))
        (Array ds size ((inst box Boolean) #f) strict! unsafe-proc)))
    (error 'dynamic-typecheck)))

(: unsafe-build-simple-array ((Option Indexes) (Option (Indexes -> (Option Float))) -> Array))
...
(: build-array ((Option (Vectorof (Option Integer)))
                (Option ((Vectorof (Option Integer)) -> (Option Float)))
                -> Array))
(define (build-array ds proc)
  (if (procedure? proc) (let ()
      (let ([ds  (check-array-shape
                  ds (lambda () (raise-argument-error 'build-array "(Vectorof Integer)" 0 ds proc)))])
        (define arr
          (unsafe-build-array ds (λ: ([js : (Vectorof (Option Integer))])
                                   (proc (vector->immutable-vector js)))))
        (array-default-strict! arr)
        arr))
    (error 'dynamic-typecheck)))

(: build-simple-array ((Vectorof (Option Integer)) (Indexes -> Float) -> Array))
...
(define-syntax-rule (make-unsafe-array-set-proc A ds set!)
  (λ: ([js : Indexes] [v : A])
    (ann (set! (unsafe-array-index->value-index ds js) v) (U #f Void))))

;; -- from 'array-constructors.rkt'

(: make-array ((Option (Vectorof (Option Integer))) (Option Float) -> Array))
...
(: unsafe-vector->array ((Option Indexes) (Option (Vectorof (Option Float))) -> Mutable-Array))
(define (unsafe-vector->array ds vs)
  (if (and (vector? ds) (vector? vs)) (let ()
      (define proc (make-unsafe-array-proc ds (λ (j) (vector-ref vs j))))
      (define set-proc (make-unsafe-array-set-proc Float ds (λ (j v) (vector-set! vs (assert j vector?) v))))
