  ...
  (cond [(not (index? dims))  (error 'array-broadcast-for-append "can't happen")]
        ...
        [else
         (let* ([dss  (map (λ: ([ds : Indexes])
                             ...
                             (vector-append ((inst make-vector (Option Integer)) (- dims dms) 1) ds))
                           dss)]
                [dks  (map (λ: ([ds : Indexes]) (assert (vector-ref ds k) integer?)) dss)]
                [dss  (map (λ: ([ds : Indexes]) (unsafe-vector-remove ds k)) dss)]
                [ds   (array-shape-broadcast dss)]
                [dss  (map (λ: ([dk : (Option Integer)]) (unsafe-vector-insert ds k dk)) dks)])
           (define new-arrs
             (map (λ: ([arr : Array] [ds : Indexes]) (array-broadcast arr ds)) arrs dss))
           (values new-arrs dks))]))

(: array-append* (-> (Option (Listof (Option Array))) Array))
(define (array-append* arrs [k 0])
  (if (and (list? arrs) (andmap array? arrs)) (let ()
      ...
      (let-values ([(arrs dks)  (array-broadcast-for-append arrs k)])
        ...
        (cond
          ...
          [else
           (: dss (Listof Indexes))
           (define dss (map (λ: ([arr : Array]) (assert (array-shape arr) vector?)) arrs))
           (: new-ds Indexes)
           ...
           (define arr*
            (unsafe-build-array
             new-ds (λ: ([js : Indexes])
                      ...
                      (define v ((assert (vector-ref old-procs jk) procedure?) js))
                      (vector-set! js k jk)
                      v)))
