...
;; Single note.
(: note (-> (Option Symbol) (Option Natural) (Option Natural) (Pairof Natural Natural)))
(define (note name octave duration)
  (cons (name+octave->note (assert name symbol?) (assert octave natural?)) (assert duration natural?)))

;; Accepts notes or pauses, but not chords.
(: synthesize-note (-> (U #f Natural)
                       Natural
                       (-> Float (-> Indexes (Option Float)))
                       Array))
...
(: sequence (-> (Option Natural)
                (Option (Listof (Option (Pairof (Option (U Natural #f)) (Option Natural)))))
                (Option Natural)
                (Option (-> Float (-> Indexes (Option Float))))
                Array))
(define (sequence n pattern tempo function)
  (: samples-per-beat Natural)
  (define samples-per-beat (quotient (* fs 60) (assert tempo natural?)))
  (array-append*
   (for*/list : (Listof Array) ([i   (in-range (assert n natural?))] ; repeat the whole pattern
                                        [note : (Option (Pairof (Option (U Natural #f)) (Option Natural)))
                                                (in-list (assert pattern list?))])
     (if (pair? note) (let ()
         (: nsamples Natural)
         (define nsamples (* samples-per-beat (assert (cdr note) natural?)))
         (synthesize-note (car note) nsamples (assert function procedure?)))
