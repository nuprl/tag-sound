...
(: longest-common-substring (-> (Option String) (Option String) String))
...
(: longest-common-sublabel (-> (Option Label) (Option Label) Label))
(define (longest-common-sublabel label-1 label-2)
  ...
  (define (main)
    ...
    (mark-up-inner-nodes! (assert (tree-root tree) node?) 0)
  ...
  (define (mark-up-inner-nodes! node depth)
    (cond [(null? (node-children node))
          ...
          [else
            (for ([child (assert (node-children node) list?)])
              (if (node? child) (let ()
  ...
  (define (absorb-children-marks! node depth)
    ;let/ec escape
      (for ([child (assert (node-children (assert node node?)) list?)])
        (if (node? child) (let ()
...
(define (path-label node)
  ...
  (define (collect-loop current-node collected-labels total-length)
    (if current-node
      (collect-loop (node-parent current-node)
                    (cons (assert (node-up-label current-node) label?) collected-labels)
                    (+ total-length
                       (label-length (node-up-label current-node))))
      (build-new-label collected-labels total-length)))
  (: vector-blit! (-> Label (Vectorof (U False Char Symbol)) Index Void))
  ...
  (define (build-new-label labels total-length)
    (: vector (Vectorof (U False Char Symbol)))
