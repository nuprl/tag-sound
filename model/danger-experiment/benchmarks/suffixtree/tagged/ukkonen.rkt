...
(provide skip-count)
(: skip-count (-> (Option Node) (Option Label) (values Node Index)))
(define (skip-count node label)
  (define l-l (label-length label))
  (unless (index? l-l) (error "skip-count"))
  (skip-count-helper (assert node node?) (assert label label?) 0 l-l))
...
(: jump-to-suffix (-> (Option Node) (values Node (U Boolean Integer))))
...
;; hasn't been set yet.
(: try-to-set-suffix-edge! (-> (Option Node) (Option Node) Void))
(define (try-to-set-suffix-edge! from-node to-node)
  (if (node? from-node) (let ()
...
(: find-next-extension-point/add-suffix-link! (-> (Option Node) (Option Label) (Option Index) (Option Index) (values (U #f Node) (U #f Index) (U #f Index))))
(define (find-next-extension-point/add-suffix-link! node label initial-i j)
  (if (index? initial-i) (let ()
         ...
         (cond [(>= i N) (values #f #f #f)]
               [else 
                (define-values (skipped-node skipped-offset)
                  (skip-count-helper suffix-node (assert label label?) K i))
...
(: extend-at-point! (-> (Option Node) (Option Index) (Option Label) (Option Index) Node))
(define (extend-at-point! node offset label i)
  ...
  (define (splice-with-internal-node! node offset label i)
    ...
    split-node)
  (main-logic (assert node node?) (assert offset index?) (assert label label?) (assert i index?)))
...
(: suffix-tree-add! (-> (Option Tree) (Option Label) Void))
(define (suffix-tree-add! tree label)
  ...
  (define (report-implicit-tree-constructed)
    (cons dummy-node 0))
  (do-construction! (assert tree suffix-tree?) (assert label label?)))
