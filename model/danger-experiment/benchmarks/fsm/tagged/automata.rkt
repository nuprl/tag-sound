...
(define (defects p0)
  (automaton DEFECT DEFECT (assert p0 natural?) defect-transitions))
...
(define (cooperates p0)
  (automaton COOPERATE COOPERATE (assert p0 natural?) cooperates-transitions))
...
(define (tit-for-tat p0)
  (automaton COOPERATE COOPERATE (assert p0 natural?) tit-for-tat-transitions))
...
(define (grim-trigger p0)
  (automaton COOPERATE COOPERATE (assert p0 natural?) grim-transitions))

(define (automaton-reset a)
  (match-define (automaton current c0 payoff table) (assert a automaton?))
...
(define (match-pair auto1 auto2 rounds-per-match)
  (match-define (automaton current1 c1 payoff1 table1) (assert auto1 automaton?))
  (match-define (automaton current2 c2 payoff2 table2) (assert auto2 automaton?))
  (define-values (new1 p1 new2 p2)
    (for/fold ([current1 : State current1]
              ...
              ([_ (in-range (assert rounds-per-match natural?))])
