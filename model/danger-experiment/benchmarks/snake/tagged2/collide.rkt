...
(define (snake-wall-collide? snk)
  (head-collide? (assert (car (assert (snake-segs (assert snk snake?)) list?)) posn?)))
...
(define (snake-self-collide? snk)
  (if (snake? snk) (let ()
      (segs-self-collide? (assert (car (assert (snake-segs snk) list?)) posn?)
                          (cdr (assert (snake-segs snk) list?))))
